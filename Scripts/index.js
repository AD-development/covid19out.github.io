...
{
	obj = await fetch("https://api.covid19india.org/data.json").then(async data=>{return (await data.json()).statewise});
...
{
	var total_confirmed=0,total_active=0,total_recovered=0,total_deaths=0;
	var str="";	
	...
	{
		if(obj[i].country != "All" && obj[i].country != "World")
		{
			if(obj[i].country==="India" && i!=obj.length-1)
			{
				total_confirmed += parseInt(indianData.confirmed);total_active+=parseInt(indianData.active);total_recovered += parseInt(indianData.recovered);total_deaths += parseInt(indianData.deaths);
				str+=getTableRow("India",indianData.confirmed,indianData.active,indianData.recovered,indianData.deaths);
				continue;
			}
			total_confirmed += parseInt(obj[i].cases.total);total_active += parseInt(obj[i].cases.active);total_recovered += parseInt(obj[i].cases.recovered);total_deaths += parseInt(obj[i].deaths.total);
			str+=getTableRow(obj[i].country,obj[i].cases.total,obj[i].cases.active,obj[i].cases.recovered,obj[i].deaths.total);
		}
	}
	str+="</tbody><tfoot>";
	str+=getTableRow("All",total_confirmed,total_active,total_recovered,total_deaths);
	str+="</tfoot></table>";
